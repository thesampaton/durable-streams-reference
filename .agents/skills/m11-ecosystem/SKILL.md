...
|----------|----------|-----|
| Framework choice | domain-web | What constraints matter? |
| Library vs build | Project requirements | What's the deployment model? |
| API design | domain-web | Who are the consumers? |
...
"FFI integration"
    ↓ Safety: unsafe blocks with SAFETY comments
...
|--------------|---------|--------|
| `extern crate` | Usually outdated (2018+) | Just `use` (exception: `extern crate alloc` still needed in `no_std`) |
...
| Vendoring everything | Maintenance burden | Trust crates.io (vendoring is valid for air-gapped/reproducible builds) |
...
| FFI safety | Rust Reference (unsafe) |
