...
"Should I use Arc or Rc?"
    ...
    ↑ Check: domain-web (performance constraints)
```
...
"Need shared ownership (single-thread)"
    ↓ Use: Rc<T>  (for shared access without ownership, just use &T)

"Need shared ownership (multi-thread)"
    ↓ Use: Arc<T>  (for shared access without ownership, just use &T)
...
|------|-----------|-------------|----------|
| `Box<T>` | Single | Same as T | Heap allocation, recursive types, trait objects |
...
| `Cell<T>` | Single | No | Interior mutability (any T; .get() needs Copy, .replace()/.take() for non-Copy) |
...
|--------------|---------|--------|
| Arc everywhere | Unnecessary atomic overhead | Use Rc for single-thread (note: Arc is normal/expected in async tokio code) |
