...
| 交易, 支付, trading, payment | **domain-web** | Audit + thread safety |
| gRPC, kubernetes, microservice | **domain-web** | Distributed tracing |
| CLI, terminal, clap | Project requirements | Usually single-thread OK |
...
"Send not satisfied for my type"
    ↑ Ask: What domain is this? Check domain-web if applicable
...
| Send/Sync in CLI | Project requirements | Is multi-thread really needed? |
...
| `Sync` | Can share references between threads | `Mutex<T>`, `i32`, most primitives |
...
├─ Type is !Send → spawn_local (requires LocalSet runtime)
...
| Error | Cause | Fix |
|-------|-------|-----|
| E0277 `Send` not satisfied | Non-Send in async | Replace !Send type (e.g., Rc→Arc), drop before .await, or spawn_local (requires LocalSet) |
...
| Use | When |
|-----|------|
| `std::sync::Mutex` | Critical section is short, never crosses `.await` (faster, no async overhead) |
| `tokio::sync::Mutex` | Need to hold lock across `.await` points |

In async code, `std::sync::Mutex` is often the right choice for simple get/set operations. Only reach for `tokio::sync::Mutex` when the lock must be held across an await point.
...
| Web handler concurrency | domain-web |
