...
| `Sync` | Safe to share `&T` between threads | `Mutex<T>`, primitives, most immutable types |
...
// - Java: data race (defined by JMM, not UB, but still buggy)
// - Python: GIL protects refcounts but not logical races
