...
```rust
// Preferred: blocking recv (iterator stops on disconnect)
for msg in rx {
    handle(msg);
}

// Or non-blocking with try_recv (WARNING: busy-waits without a sleep/yield)
loop {
    match rx.try_recv() {
        Ok(msg) => handle(msg),
        Err(TryRecvError::Empty) => std::thread::yield_now(),  // avoid 100% CPU spin
