...
### Borrowing Data in Tasks
```rust
// Preferred: use JoinSet with owned/cloned data
use tokio::task::JoinSet;

async fn process_all(data: Vec<u32>) {
    let mut set = JoinSet::new();
    for item in data {
...
// For borrowing without 'static, use std::thread::scope (blocking)
// or clone the data into each task
```
...
async fn serve_with_shutdown(shutdown: impl Future) {
    ...
    // Pin the future so it can be used in select!
    tokio::pin!(shutdown);
