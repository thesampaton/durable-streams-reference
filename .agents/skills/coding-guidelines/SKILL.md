...
| Static/const naming | `SCREAMING_SNAKE_CASE` for both `static` and `const` (no prefix) |
...
| `std::sync::mpsc` | `crossbeam::channel` (not deprecated, but crossbeam has more features) | - |
| `std::sync::Mutex` | `parking_lot::Mutex` (not deprecated, but parking_lot is faster uncontended) | - |
