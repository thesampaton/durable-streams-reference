...
"Need mutable access from &self"
    ↓ T: Copy → Cell<T> (.get()/.set())
    ↓ T: !Copy → Cell<T> (.replace()/.take()) or RefCell<T>
    ↓ Initialize once → OnceLock (thread-safe) or OnceCell (single-thread)
...
| `Cell<T>` | No | Zero | Any T; .get() for Copy, .replace()/.take() for non-Copy |
...
| Scenario | Choose |
|----------|--------|
| T: Copy, single-thread | `Cell<T>` (.get()/.set()) |
| T: !Copy, single-thread | `Cell<T>` (.replace()/.take()) or `RefCell<T>` |
...
| Mutex for single-thread | Unnecessary overhead | RefCell (but in async/tokio, Mutex is often required because RefCell is !Send) |
