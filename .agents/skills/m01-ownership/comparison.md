...
| Aspect | Rust | C++ |
|--------|------|-----|
| Default | Move semantics | Copy semantics (move opt-in since C++11) |
...
// std::cout << a;  // valid but unspecified state (compiles, value indeterminate)
```
...
| `Arc<T>` | `std::shared_ptr<T>` (both atomic) | Thread-safe shared |
...
| Aspect | Rust | Go |
|--------|------|-----|
| Memory | Stack + heap, explicit | GC + stack (escape analysis) |
...
5. **Affine Types**: Values can be moved at most once (can be read/borrowed multiple times before moving)
