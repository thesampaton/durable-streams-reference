...
### Error Pattern
```rust
// E0716 occurs when a temporary is used in a context where it cannot
// be lifetime-extended (e.g., inside a more complex expression or struct init)
let r: &str = some_fn(&String::from("hello"));  // temporary dropped after statement
println!("{}", r);  // ERROR: temporary no longer alive

// Note: simple `let r: &str = &String::from("hello");` actually compiles
// in modern Rust due to temporary lifetime extension rules.
```
...
```rust
let r: String = some_fn_owned(String::from("hello"));
println!("{}", r);
```
