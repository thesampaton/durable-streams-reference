...
let mut mutable_items = vec![1, 2, 3, 4, 5];
for item in &mut mutable_items {
    *item *= 2;
}
...
```rust
// Allocates new String (handles full Unicode)
fn make_uppercase(s: &str) -> String {
    s.to_uppercase()
}

// In-place when you own the data AND know input is ASCII-only
// WARNING: make_ascii_uppercase only handles a-z, not Unicode (e.g., ß → SS)
// Use only when you can guarantee ASCII input
fn make_ascii_upper(mut s: String) -> String {
