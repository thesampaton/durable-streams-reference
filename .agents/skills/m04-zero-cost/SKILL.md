...
fn process(x: impl Display) { }      // argument position (callers can't turbofish)
fn process<T: Display>(x: T) { }     // explicit generic (allows turbofish, same-type constraints)
fn get() -> impl Display { }         // return position (one concrete type, opaque to caller)

// Dynamic dispatch - type determined at runtime
fn process(x: &dyn Display) { }      // reference
fn process(x: Box<dyn Display>) { }  // owned

// In traits (stable since Rust 1.75 - RPITIT)
trait MyTrait {
    fn items(&self) -> impl Iterator<Item = u32>;  // return impl Trait in trait methods
    async fn fetch(&self) -> Result<Data, Error>;  // async fn in traits (no async-trait crate needed)
}
```
...
- Doesn't require `Self: Sized` as a supertrait
- Methods that return `Self` or have type parameters are excluded via `where Self: Sized`
- No methods with generic type parameters (lifetime parameters are OK)
- Individual methods can opt out of the vtable with `where Self: Sized`
