...
"How fast does this need to be?"
    ...
    ↑ Check: domain-web (latency requirements)
    ↑ Check: Business requirements (acceptable response time)
```

| Question | Trace To | Ask |
|----------|----------|-----|
| Latency requirements | domain-web | What's acceptable response time? |
| Throughput needs | domain-web | How many requests per second? |
| Memory constraints | Project requirements | What's the memory budget? |
...
| String concat in loop (`result = result + &s`) | O(n^2) | `push_str` in loop, `join`, or `String::with_capacity` + `push_str` |
