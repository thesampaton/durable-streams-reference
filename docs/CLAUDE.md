...
|-----------|----------------|------------------|-----------------|
| CORS not covered by conformance | Allow all origins by default, configurable via `DS_HTTP__CORS_ORIGINS` | Aligns with typical development defaults, restrictable in production | Test OPTIONS preflight with various origin headers |
