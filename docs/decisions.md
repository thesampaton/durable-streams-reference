...
| CORS: allow all origins by default, configurable via `DS_SERVER__CORS_ORIGINS` | Deployment defaults | CORS is not part of protocol semantics. Default allow-all suits development; production deployments restrict via env var or auth proxy. | 2026-02-09 |
